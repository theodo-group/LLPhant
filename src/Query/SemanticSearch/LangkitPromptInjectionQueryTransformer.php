...
class LangkitPromptInjectionQueryTransformer implements QueryTransformer
{
    ...
    public function transformQuery(string $query): array
    {
        ...
        foreach ($this->jailbreakPromptDocuments as $document) {
            if ($this->distance->measure($document->embedding, $embeddedQuery) < $this->nearDistance) {
    ...
    {
        if (\count($this->jailbreakPromptDocuments) === 0) {
            $multiQuery = new MultiQuery($this->chat, 1);
            $this->jailbreakPromptDocuments = DocumentUtils::documents(...$this->jailbreakPromptDocuments);
            $this->nearDistance = 0;
            foreach ($this->jailbreakPromptDocuments as $document) {
                $document->embedding = $this->embeddingGenerator->embedText($document->content);
                $comparisonPrompt = $multiQuery->transformQuery($document->content)[1];
                $comparisonDistance = $this->distance->measure($document->embedding, $this->embeddingGenerator->embedText($comparisonPrompt));
                if ($comparisonDistance > $this->nearDistance) {
                    $this->nearDistance = $comparisonDistance;
                }
            }
        }
    }
